...
const BookingSummaryFooter: React.FC<FooterProps> = ({ booking, onNext, isLastStep, choiceButtons }) => {
  const isTripPackage = booking.selectedPackageId === 'trip';
  ...
  const basePrice = isTripPackage ? 0 : priceFor('core-8h');
  const addonsPrice = booking.services
    .filter(s => s.selected && s.type === 'ADDON')
    .reduce((acc, s) => acc + priceFor(s.id), 0);

  const totalPrice = isTripPackage ? addonsPrice : basePrice + addonsPrice;
  ...
  if (isLastStep) {
    if (isTripPackage) {
      buttonText = `Hold $${tripHoldAmount} Pre-Auth`;
    } else if (booking.isSplitBill) {
        ...
        {choiceButtons ? (
          <div className="flex flex-col gap-2">
            ...
            >
              <div className="min-w-0 max-w-[62%] leading-tight">
                ...
                <div className="text-[11px] text-white/80 truncate">
                  {isTripPackage ? 'Pre-authorization hold' : 'Split with group'}
                </div>
              </div>
              <div className="text-right shrink-0 leading-tight">
                <div className="text-base font-bold whitespace-nowrap">${(isTripPackage ? tripHoldAmount : displayedSplitPrice).toFixed(2)}</div>
                <div className="text-[11px] text-white/80">{isTripPackage ? 'pre-auth' : 'per person'}</div>
              </div>
            </button>
            <button
            ...
            >
              <div className="min-w-0 max-w-[62%] leading-tight">
                ...
                <div className="text-[11px] text-white/80 truncate">
                  {isTripPackage ? 'Pre-authorization hold' : 'Pay for everyone'}
                </div>
              </div>
              <div className="text-right shrink-0 leading-tight">
                <div className="text-base font-bold whitespace-nowrap">${(isTripPackage ? tripHoldAmount : totalPrice).toFixed(2)}</div>
                <div className="text-[11px] text-white/80">{isTripPackage ? 'pre-auth' : 'total'}</div>
              </div>
            </button>
          </div>
        ) : (
          <div className="flex items-center justify-between gap-4">
            <div className="flex flex-col">
              {isTripPackage ? (
                <>
                  <span className="text-xs font-medium text-slate-500 mb-0.5">Point-to-point metered</span>
                  <div className="flex items-baseline gap-1">
                    <span className="text-2xl font-bold text-slate-900">${tripBaseRate.toFixed(0)}</span>
                    <span className="text-xs text-slate-400 font-medium">/ trip</span>
                  </div>
                  <span className="text-[10px] text-slate-400">Up to 30 km per trip · Pre-auth ${tripHoldAmount}</span>
                </>
              ) : (
                <>
